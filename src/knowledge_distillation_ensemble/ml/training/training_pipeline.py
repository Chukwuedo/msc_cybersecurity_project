...
from .teacher_model import (
    train_teacher, predict_logits, logits_to_probs, 
    calibrate_temperature, logits_to_calibrated_probs
)
...
def load_from_splits(
    ...
    unseen_path: Optional[Path], 
    target_col: str
) -> Tuple[pl.DataFrame, pl.DataFrame, pl.Series, pl.Series, Optional[pl.DataFrame], Optional[pl.Series]]:
...
def run_task(
    *,
    ...
    X_train_pl, X_test_pl, y_train_pl, y_test_pl, X_unseen_pl, y_unseen_pl = load_from_splits(
        split, unseen_path, target_col
    )
    ...
    if X_unseen_np is not None and y_unseen is not None:
        ...
        unseen_results = {
            "teacher": evaluate(task, y_unseen, teacher_unseen_pred, teacher_unseen_probs),
            "student": evaluate(task, y_unseen, student_unseen_pred, student_unseen_proba),
            "benchmark": evaluate(task, y_unseen, benchmark_unseen_pred, benchmark_unseen_proba),
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--unseen", 
        default="cicdiad2024", 
        help="Unseen evaluation dataset"
    )
    parser.add_argument(
        "--standard", 
        action="store_true", 
